...
select *, iif(Price>1000,'Expensive','Affordable') CategorybyPrice from Products
...
--14. Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' if SaleAmount BETWEEN 200 AND 500, 
--and 'Low Tier' otherwise. (From Sales table)
select *, case when SaleAmount > 500 then 'Top Tier'
			   when SaleAmount > 200 then 'Mid Tier'
			   else 'Low Tier' end CategorybySaleAmount from Sales 
...
--Use orders table. Result set should show  customerid, quantity and discount percentage. The discount should be applied as follows:
--        1 item: 3% 
--        Between 1 and 3 items : 5% 
--        Otherwise: 7% 
select *, case when Quantity<=1 then '3%'
			   when Quantity<=3 then '5%'
			   else '7%' end CategorybyQuantity from orders
--Necessary tables:
