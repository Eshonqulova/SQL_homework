...
select 
	count(distinct category) dist_category
from Products
...
select ProductID, avg(SaleAmount) AVG_sale from Sales
group by ProductID
...
group by 
	CustomerID
having 
	max(TotalAmount)>50 and min(TotalAmount)>50
...
select 
	...
	sum(SaleAmount) total_sum, 
	count(distinct SaleAmount) dist_salecount  
from Sales
group by 
	year(SaleDate),
	month(SaleDate)
having 
	count(distinct SaleAmount)>=8
