...
select Name EmployeeName, Salary, DepartmentName from Employees join Departments on Employees.DepartmentID = Departments.DepartmentID where Salary > 5000
...
select FirstName, LastName, OrderDate from Orders join Customers on  Orders.CustomerID = Customers.CustomerID where OrderDate>'2023-01-01' 
...
select * from Employees
select * from Departments
select Name EmployeeName, DepartmentName from Employees left join Departments on Employees.DepartmentID = Departments.DepartmentID group by Name, DepartmentName
...
select SupplierName, ProductName from Products full join Suppliers on  Products.SupplierID = Suppliers.SupplierID 
...
select  Orders.OrderID, OrderDate, PaymentDate, Amount from Payments join Orders on Orders.OrderID = Payments.OrderID
...
select * from Employees
select Name EmployeeName , ManagerID ManagerName from Employees  where ManagerID is not null order by ManagerID   
...
select  Name StudentName, CourseName
from Enrollments join Students on Enrollments.StudentID = Students.StudentID
join Courses on Enrollments.CourseID = Courses.CourseID 
order by 
	case 
		when Courses.CourseName = 'Math101' then 0
		else 1
	end
...
select * from Customers 
select * from Orders
select FirstName, LastName, Quantity from Customers join Orders on Customers.CustomerID = Orders.CustomerID where Quantity>3

--9. **Using the `Employees` and `Departments` tables**, write a query to list employees working in the 'Human Resources' department.  
--   🔁 *Expected Output:* `EmployeeName`, `DepartmentName`  
select * from Employees 
select * from Departments
select Name EmployeeName, DepartmentName from Employees join Departments on Employees.DepartmentID = Departments.DepartmentID where Employees.DepartmentID=2
...
select * from Employees 
select * from Departments
select DepartmentName, count(EmployeeID) EmployeeCount from Employees join Departments on Employees.DepartmentID = Departments.DepartmentID  group by Departments.DepartmentName having count(Employees.EmployeeID)>5
...
select c.FirstName, c.LastName, count(o.CustomerID) TotalOrders from Customers c join Orders o on c.CustomerID = o.CustomerID group by   FirstName, LastName having count(o.CustomerID)>=1
...
select * from Employees 
select * from Departments
select e.Name EmployeeName, DepartmentName from Employees e join Departments d on e.DepartmentID = d.DepartmentID where ManagerID is not null
 
--14. **Using the `Employees` table**, write a query to find pairs of employees who report to the same manager.  
--   🔁 *Expected Output:* `Employee1`, `Employee2`, `ManagerID`
select * from Employees 
select e.Name Employee1, b.Name Employee2, e.ManagerID from Employees e  join Employees b on e.ManagerID = b.ManagerID where e.EmployeeID <> b.EmployeeID and e.ManagerID is not null order by e.ManagerID asc

--15. **Using the `Orders` and `Customers` tables**, write a query to list all orders placed in 2022 along with the customer name.  
--   🔁 *Expected Output:* `OrderID`, `OrderDate`, `FirstName`, `LastName`
select * from Customers 
select * from Orders
select o.OrderID, o.OrderDate, c.FirstName, c.LastName from Customers c join Orders o on c.CustomerID = o.CustomerID where o.OrderDate>'2022-01-01' and o.OrderDate<'2023-01-01'
...
select * from Employees 
select * from Departments
select Name EmployeeName, Salary, DepartmentName from Employees e join Departments d on e.DepartmentID = d.DepartmentID  where e.Salary>60000 
...
select * from Products 
select * from Orders
select  p.ProductID, p.ProductName from Products p left join Orders o on o.ProductID=p.ProductID where o.ProductID is null
...
--   🔁 *Expected Output:* `EmployeeName`, `Salary`
 select * from Employees 
select Name EmployeeName, Salary from Employees e where Salary>(select avg(Salary) from Employees)
...
select * from Orders
select * from Payments
select Orders.OrderID, OrderDate from Orders left join Payments on Orders.OrderID = Payments.OrderID where Orders.OrderDate<'2020-01-01' and Payments.OrderID is null

--22. **Using the `Products` and `Categories` tables**, write a query to return products that do not have a matching category.  
--   🔁 *Expected Output:* `ProductID`, `ProductName`
select * from Products
select * from Categories
select ProductID, ProductName from Products p left join Categories c on p.Category = c.CategoryID where p.Category is null
