...
BEGIN
    ...
    -- Yangilangan xodimlarni ko‘rsatish
    SELECT 
        EmployeeID,
        FirstName,
        LastName,
...
-- Agar faqat `Current`da bo‘lsa, `New`da bo‘lmasa: o‘chirish
...
SELECT 
    s.user_id,
    ROUND(
        COALESCE(1.0 * SUM(CASE WHEN c.action = 'confirmed' THEN 1 ELSE 0 END) / COUNT(c.user_id), 0)
