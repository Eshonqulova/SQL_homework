...
--Here are 27 homework tasks for Lesson 4, categorized into easy, medium, and hard levels. These tasks cover: ✅ Filtering and Ordering Data ✅ Using SELECT TOP, OFFSET-FETCH ✅ Using ISNULL and COALESCE ✅ Order By (ASC, DESC) ✅ Where Clause Filtering ✅ LIKE operator ✅ Wildcard Characters ✅ ANY and ALL Operators
...
-- ... Qator 6-30 gacha shu tartibda davom eting!
...
select EmployeeID,FirstName, LastName, DepartmentName, Salary, HireDate, Age, isnull(Email,'noemail@example.com') newEmail, Country from Employees
...
select EmployeeID,coalesce(FirstName,LastName) Name, DepartmentName, Salary, HireDate, Age, Email , Country from Employees
...
--15.	Use OFFSET-FETCH to select rows 10 to 20 from the Employees table, ordered by Salary DESC.
select * from Employees
...
--19.	Use ORDER BY to display a list of customers ordered by City in ascending and 
--PostalCode in descending order. Use customers table
...
-- employees dajvalida departmentname ustunida sales degan bo'lim yo'qligi uchun yana bitta kod yozaman  departmentname=HR uchun 
select * from Employees
...
select getdate() as [current_date] -- date+soat
