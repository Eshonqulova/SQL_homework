...
INSERT INTO DML (SequenceNumber, String) VALUES
(1,'SELECT'),
(2,'Product,'),
(3,'UnitPrice,'),
(4,'EffectiveDate'),
...
select concat(EMPLOYEE_ID,'-',FIRST_NAME,' ',LAST_NAME)  name from Employees
where EMPLOYEE_ID=100

select concat_ws(' ',EMPLOYEE_ID,FIRST_NAME,LAST_NAME)  name from Employees
where EMPLOYEE_ID=100
...
select *,replace(PHONE_NUMBER,124,999) [new phone_number] from employees
...
select FIRST_NAME, len(ltrim(FIRST_NAME)) [length_name]  from Employees
where patindex('[A,J,M]%',FIRST_NAME)=1
order by FIRST_NAME 

SELECT FIRST_NAME AS [First Name], LEN(FIRST_NAME) AS [Name Length] FROM Employees
...
select *,case when Max1>Max2 and Max1>Max3 then Max1
				  when Max2>Max1 and Max2>Max3 then Max2
				  else Max3 end [highest value]
 from TestMax
...
select *,coalesce(ssn,passportid,itin) [first value] from person
...
substring(FullName, CHARINDEX(' ', FullName)+1, charindex(' ',FullName,CHARINDEX(' ', FullName)+1)-CHARINDEX(' ',FullName)-1) Middlename,
RIGHT(FullName,CHARINDEX(' ', reverse(FullName))-1) Lastname,
SUBSTRING(FullName,CHARINDEX(' ', fullname)+1,len(fullname)-CHARINDEX(' ',reverse(FullName))-CHARINDEX(' ', fullname))
,CHARINDEX(' ', FullName)
,reverse(FullName),
len(fullname),
...
--2. For every customer that had a delivery to California, provide a result set of the customer orders that were delivered to Texas.
--( Table)

select * from Orders
where DeliveryState='TX' and CustomerID in (select distinct CustomerID from Orders where DeliveryState='CA')

--3. Write an SQL statement that can group concatenate the following values.(DML Table)

SELECT  STRING_AGG(rtrim(String),', ') AS Concatenated FROM DML

--4. Find all employees whose names (concatenated first and last) contain the letter "a" at least 3 times.

select EMPLOYEE_ID,FIRST_NAME+' '+LAST_NAME [employees_names] from Employees
...
select 
	...
	sum(case when DATEPART(month,HIRE_DATE) > 8 then 1 else 0 end) as category,
	cast(cast(sum(case when DATEPART(month,HIRE_DATE) > 8 then 1 else 0 end) as decimal(10,2))/COUNT(EMPLOYEE_ID) *100.0 as decimal(10,1)) as pct_august_employee
from Employees
...
;with cte as(
select JobDescription,max(MissionCount) mx,min(MissionCount) mn from personal
group by JobDescription)
select cte.JobDescription,b.SpacemanID max,c.SpacemanID min from cte
join personal b on cte.JobDescription=b.JobDescription and cte.mx=b.MissionCount
join personal c on cte.JobDescription=c.JobDescription and cte.mn=c.MissionCount

--## Difficult Tasks
--1. Write an SQL query that counts the uppercase letters, lowercase letters, numbers, and other characters from the given string 'tf56sd#%OqH' into separate columns.

with cte as(
select 'tf56sd#%OqH' letters, 1 id
union all
select letters,id+1 from cte where len(letters)>id
),
cte2 as(
select letters,substring(letters,id,1) sb from cte)
select sum(case when ASCII(sb) between 65 and 90 then 1 else 0 end) UpperCase,
	   sum(case when ASCII(sb) between 97 and 122 then 1 else 0 end) LowerCase,
	   sum(case when ASCII(sb) between 48 and 57 then 1 else 0 end) Numbers,
	   len(letters)-sum(case when ASCII(sb) between 65 and 90 then 1 else 0 end)-sum(case when ASCII(sb) between 97 and 122 then 1 else 0 end)-sum(case when ASCII(sb) between 48 and 57 then 1 else 0 end) [Others] from cte2
group by letters

--2. Write an SQL query that replaces each row with the sum of its value and the previous rows' value. (Students table)

select a.StudentID,a.Grade ,sum(b.Grade) from Students a left join Students b on a.StudentID>=b.StudentID
group by a.StudentID,a.Grade
...
select Equation,sum(cast(value as int)) from Equations
...
select case when PlayerA>PlayerB then PlayerB else PlayerA end PlayerA,
case when PlayerA>PlayerB then PlayerA else PlayerB end PlayerB,sum(score) score from PlayerScores
group by case when PlayerA>PlayerB then PlayerB else PlayerA end,
